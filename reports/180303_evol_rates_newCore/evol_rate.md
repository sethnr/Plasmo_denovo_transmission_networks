...
![plot of chunk unnamed-chunk-4](figure/chrs-unnamed-chunk-4-1.png)
...
extDists <- as.matrix(distmatExt)
coreDists <- as.matrix(distmatCore)
allDists <- as.matrix(distmatAll)
net$direct<-T
netAll <- net
...
test <- getEvolRates(varNets[[1]],extDists)
...
```r
extr1 <- getEvolRates(varNets[[1]],extDists,ances=F)  
corer1 <- getEvolRates(varNets[[1]],coreDists,ances=F) 
allr1 <- getEvolRates(varNets[[1]],allDists,ances=F) 
extr1 <- subset(extr1,years>0)
corer1 <- subset(corer1,years>0)
allr1 <- subset(allr1,years>0)
...
coreplot <- ggplot(subset(corer1,years>0),aes(x=years,y=rate)) + 
  ...
  geom_label(aes(label=paste(round(mean(corer1$rate,na.rm=T),2)," (+/- ",round(sd(corer1$rate,na.rm=T),1),")",sep=""),
                 y=mean(corer1$rate,na.rm=T)+0.2,x=max(years)),size=5,inherit.aes=F)+
  ggtitle("evol rate, core genome") + ylab("INDELs/mth/gen") + xlab("years")  + ylim(-2,9) + #xlim(0,7.5) +
  theme(legend.position="none")
#INACCESSIBLE ONLY
extplot <- ggplot(subset(extr1,years>0),aes(x=years,y=rate)) + 
  geom_boxplot(aes(group=years),fill=NA) + geom_hline(aes(yintercept=mean(rate,na.rm=T)),linetype=3) + 
  geom_point(aes(colour=to,shape=from),position=position_jitter(width=0.5),size=3)+
  geom_label(aes(label=paste(round(mean(extr1$rate,na.rm=T),2)," (+/- ",round(sd(extr1$rate,na.rm=T),1),")",sep=""),
                 y=mean(extr1$rate,na.rm=T)+1.0,x=max(years)),size=5,inherit.aes=F)+
  ggtitle("evol rate, extended genome") + ylab("SNPs/mth/gen") + xlab("years") + ylim(-2,9) #+ xlim(0,7.5)
#splot
legend <- g_legend(extplot)
grid.arrange(coreplot, extplot + theme(legend.position="none"),legend, nrow=1,widths=c(10,10,1),heights=c(1))
```
...
```r
#SNPs + INDELs
ggplot(subset(allr1,years>0),aes(x=years,y=rate)) + 
  geom_boxplot(aes(group=years),fill=NA) + geom_hline(aes(yintercept=median(rate,na.rm=T)),linetype=3) + 
  geom_point(aes(colour=to,shape=from),position=position_jitter(width=0.5),size=3) +
  geom_label(aes(label=paste(round(mean(allr1$rate,na.rm=T),2)," (+/- ",round(sd(allr1$rate,na.rm=T),1),")",sep=""),
                 y=mean(allr1$rate,na.rm=T)+0.1,x=max(years)),size=5,inherit.aes=F)+
...
allrate <- round(mean(allr1$rate)*12,3)
corerate <- round(mean(corer1$rate)*12,3)
extrate <- round(mean(extr1$rate)*12,3)
```
...
sppsubs$region<-"all"
sppsubs <- rbind(sppsubs,
      c("P.falciparum","P.falciparum (all)", "Eukaryote",pfgsize,signif(allrate/pfgsize,2),allrate,"among",8,"ours","SNP+INDEL","all"),
      c("P.falciparum","P.falciparum (ext)","Eukaryote",pfgsize - pfmingsize,signif(extrate/pfiagsize,2),allrate,"among",8,"ours","SNP+INDEL","extended"),
      c("P.falciparum","P.falciparum (core)", "Eukaryote",pfmingsize,signif(corerate/pfmingsize,2),corerate,"among",8,"ours","SNP+INDEL","core")
      )
...
ggplot(subset(sppsubs, Label %in% dotsubs),aes(x=gsize,y=subrate),) +  
  ...
  geom_point(aes(label=Pathogen,colour=Group),size=5) + 
...
"EBOV (Ebola)", "HIV-M", 
"HFLUV (H1N1)", "P.falciparum (ext)",
"P.falciparum (core)")
labelSamps <- c("M. tuberculosis", "S. aureus (MRSA)","H. pylori", 
"EBOV (Ebola)", "HIV-M", 
"HFLUV (H1N1)", "P.falciparum (all)")
midLabelSamps <- c("P.falciparum (core)")

sppsubs$region <- factor(sppsubs$region,levels=c("all","extended","core"))
sppsubs$fill <- factor(paste(sppsubs$region, sppsubs$Group),levels=c("all Bacteria","all RNA virus","all Eukaryote","extended Eukaryote","core Eukaryote"),ordered=T)

fillscale <- scale_fill_manual(values=c("all Bacteria"="#F6766D","all RNA virus"="#0BB939","all Eukaryote"="#609CFF","core Eukaryote"="#2D81FC","extended Eukaryote"="#85B5F9"))
...
barlegend <- g_legend(
  ...
  geom_label(data=subset(sppsubs,Label %in% midLabelSamps),aes(label=newmut, y=(subrate * gsize)))+
  scale_alpha_manual(values=c("all"=1,"core"=1,"extended"=0.7))+
...
barplot <- ggplot(subset(sppsubs,Label %in% barSamps),aes(x=Pathogen,y=subrate * gsize),) +
#  geom_bar(aes(fill=Group,alpha=region),stat = "identity",size=3) +
  geom_bar(aes(fill=fill),stat = "identity",size=3) +
  geom_label(data=subset(sppsubs,Label %in% labelSamps),aes(label=newmut))+
  geom_label(data=subset(sppsubs,Label %in% midLabelSamps),aes(label=newmut, y=(subrate * gsize)))+
#  scale_alpha_manual(values=c("all"=1,"accessible"=1,"inaccessible"=0.5))+
  fillscale+
  ylab("mutation rate (muts / genome / year)")+
  theme(panel.background = element_rect(fill = F),axis.text.x = element_text(angle = 90, hjust = 1))+
  theme(legend.position="none")

grid.arrange(barplot,barlegend,widths=c(10,1))
```

![plot of chunk unnamed-chunk-11](figure/chrs-unnamed-chunk-11-2.png)
...
```
## Pathogen	Label	Group	gsize	subrate	subrate_genome	Host.scale	Date.range..yrs.	Source	vartype	region	newmut	fill
## 1	H. pylori	H. pylori	Bacteria	1600000	1.38e-05	22	among	15	[S5]	SNP	all	2.4 wks	all Bacteria
## 2	M. tuberculosis	M. tuberculosis	Bacteria	4400000	6.8e-08	0.3	among	10	[S9]	SNP	all	3.3 yrs	all Bacteria
## 3	S. aureus	S. aureus (MRSA)	Bacteria	2900000	3.3e-06	9.57	among	25	[S13]	SNP	all	1.3 mths	all Bacteria
## 4	S. pneumoniae	S. pneumoniae	Bacteria	2200000	1.6e-06	3.52	among	25	[S15]	SNP	all	3.4 mths	all Bacteria
## 5	V. cholerae	V. cholerae	Bacteria	4e+06	8.3e-07	3.32	among	60	[S16]	SNP	all	3.6 mths	all Bacteria
## 6	HCV	HCV (Hep C)	RNA virus	9000	0.001	9	among	35	[S20]	SNP	all	1.3 mths	all RNA virus
## 7	EBOV	EBOV (Ebola)	RNA virus	19000	0.002	9	among	35	[S20]	SNP	all	1.4 wks	all RNA virus
## 8	HIV-M	HIV-M	RNA virus	10000	0.0033	33	among	50	[S21]	SNP	all	1.6 wks	all RNA virus
## 9	HFLUV	HFLUV (H1N1 2009)	RNA virus	13600	0.0036	48.96	among	1	[S22]	SNP	all	1.1 wks	all RNA virus
## 10	HFLUV	HFLUV (H1N1)	RNA virus	13600	0.002	27.2	among	60	[S23]	SNP	all	1.9 wks	all RNA virus
## 11	RABV	RABV	RNA virus	12000	2e-04	2.4	among	22	[S24], Biek, unpubl.	SNP	all	5 mths	all RNA virus
## 12	FMDV	FMDV	RNA virus	8100	0.0013	9	among	35	[S20]	SNP	all	1.1 mths	all RNA virus
## 13	P.falciparum	P.falciparum (all)	Eukaryote	18446000	1.9e-06	35.068	among	8	ours	SNP+INDEL	all	1.5 wks	all Eukaryote
## 14	P.falciparum	P.falciparum (ext)	Eukaryote	3266000	6.7e-06	35.068	among	8	ours	SNP+INDEL	extended	2.4 wks	extended Eukaryote
## 15	P.falciparum	P.falciparum (core)	Eukaryote	15180000	8.7e-07	13.194	among	8	ours	SNP+INDEL	core	3.9 wks	core Eukaryote
```

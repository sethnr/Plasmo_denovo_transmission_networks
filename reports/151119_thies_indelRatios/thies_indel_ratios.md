...
```r
scol <- scale_color_manual(values=c(NA,"red"))
ggplot(distances,aes(x=Var1,y=Var2,fill=total)) + geom_tile() + scale_fill_gradient(trans="log")
```
...
```r
ggplot(subset(distances,related=="RELATED"),aes(x=Var1,y=Var2,fill=ratio,colour=ratioP < 0.05)) + geom_tile(size=1) + scol + scale_fill_gradient(trans="log")
```
...
ggplot(distances,aes(x=snp,y=indel,colour=related,group=related,label=clades)) + geom_text(size=4) + unrelatedLM + xlim(0,400) + ylim(0,400)
```

...
snpindel <- read.table("SNP_INDEL_Pf3kGhana_1K.SI-RATIO.tab.txt")
snpindel[lower.tri(snpindel)]<-NA
snpindel$from = colnames(snpindel)
...
```{r fig.width=13, fig.height=8}
dist <- read.table("SNP_INDEL_Pf3kGhana_1K.DIST.tab.txt",sep="\t")
dist$from = colnames(dist)
dist <- melt(dist)
...
# snpindel <- snpindel[dist$dist!=0,]


cf <- merge(dist,snpindel,by=c("from","variable"))

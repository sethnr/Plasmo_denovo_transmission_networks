...
ggplot(subset(callCf,from != to),aes(y=snp,x=indel)) + geom_point() + geom_smooth(method = lm,se = F,linetype=2) +
  facet_grid(. ~ prog)+
  ggtitle("SNP:INDEL correlation")+
  geom_label(data=corrs,aes(y=100,x=750,label=paste("r = ",round(SI,2))),size=5) +
  theme(legend.position="bottom") + xlab("INDEL distances") + ylab("SNP distances")
...
ggplot(subset(callCf,from != to),aes(y=both,x=extended)) + geom_point() + geom_smooth(method = lm,se = F,linetype=2) +
  facet_grid(. ~ prog)+
...
opts_chunk$set(fig.width=12, fig.height=13,dev=c('png','postscript'),warning=F)
...
ggplot(data=subset(callCfSIAll,prog=="disco"),aes(x=from,y=to,fill=value,label=value)) + 
  ...
  geom_text(data=subset(callCfSIAll,prog=="disco" & variable=="snp" & related==F), size=2.5,colour="black") + 
  geom_text(data=subset(callCfSIAll,prog=="disco" & variable=="indel" & related==F),aes(x=to,y=from,label=value), size=2.5,colour="black") + 
  iscol + vxlab + vxleg
```